...
#ifndef Final_Project_DCircularLinkedList_h
#define Final_Project_DCircularLinkedList_h
#include "LinkedList.h"

using namespace gva;

template <class T>
class DCircularLinkedList :  public LinkedList<T> {
...
public:
    ...
    friend std::ostream & operator <<(std::ostream &, DCircularLinkedList<Tn> &);
    
};

template <class T>
DCircularLinkedList<T>::~DCircularLinkedList()
{
    this->clear();
}
...
template  <class T>
void DCircularLinkedList<T>::insert(Node<T> * newnode, int position)
{
...
template  <class T>
Node<T> * DCircularLinkedList<T>::remove(int position)
{
...
template  <class T>
Node<T> *  DCircularLinkedList<T>::at(int position)
{
    /* Cuando la lista está vacía o position es inválida */
    if (this->empty() || (position < 0 || position >= this->_size )) {
        return nullptr;
...
template  <class T>
void DCircularLinkedList<T>::clear()
{
    /* Cuando la lista está vacía */
...
template <class T>
std::ostream & operator <<(std::ostream & os, DCircularLinkedList<T> & list)
{
    for (auto node : list )
    {
        os << node << std::endl;
