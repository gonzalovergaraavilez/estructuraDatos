...
namespace vcn {
    ...
    void LinkedList<T>::insert(Node<T> * newnode, int position)
    {
        ...
        /* Cuando se inserta en cualquier posición diferente del inicio */
            ...
            Node<T> * tmpPrevious = this->at(position-1);
            Node<T> * tmpNext = this->at(position+1);
            
            newnode->setNext(tmpPrevious->getNext());
            newnode->setPrevious(tmpPrevious);
            
            if(tmpNext!= nullptr) {
                tmpNext->setPrevious(newnode);
            }
            
            tmpPrevious->setNext(newnode);
        }
    ...
    Node<T> * LinkedList<T>::remove(int position)
    {
        ...
        /* Eliminar cualquier otro nodo */
            ...
            Node<T> * next = this->at(position+1);
